...
test('GET_JOB_LIST_3', { tag: ["@get_job_list"] }, async ({ page }) => {
    ...
    console.log(r.data.response.status == 'BAD REQUEST' ? 'STATUS CHECK PASSED ✅' : `STATUS CHECK FAILED ❌ \nEXPECTED STATUS: BAD REQUEST \nACTUAL STATUS: ${JSON.stringify(r.data.response.status)}`);
    ...
    expect(r.data.response.status).toEqual('BAD REQUEST')
