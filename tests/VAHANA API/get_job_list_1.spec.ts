...
test.beforeEach(async ({ page }) => {
    ...
    await page.waitForFunction(() => window.doAuthentication !== undefined);
    ...
    if (!await Hooks.isAthenticated()) {
        await Hooks.authenticate(page, body, headers, env);
    }
})


test('GET_JOB_LIST_1', { tag: ["@get_job_list_1"] }, async ({ page }) => {
