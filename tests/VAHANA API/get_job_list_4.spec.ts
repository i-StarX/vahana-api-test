...
test('GET_JOB_LIST_4', { tag: ["@get_job_list"] }, async ({ page }) => {
    ...
    const expected = { "error": "Invalid jobId format" };
    const responseArray = r.data.response.response;
    const isResponsePresent = responseArray.some(actual =>
        actual.error === expected.error
    );

    console.log(r.data.response.status == 'UNPROCESSABLE ENTITY' ? 'STATUS CHECK PASSED ✅' : `STATUS CHECK FAILED ❌ \nEXPECTED STATUS: UNPROCESSABLE ENTITY \nACTUAL STATUS: ${JSON.stringify(r.data.response.status)}`);
    ...
    expect(r.data.response.status).toEqual('UNPROCESSABLE ENTITY')
