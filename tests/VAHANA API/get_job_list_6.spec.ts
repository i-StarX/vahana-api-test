...
test('GET_JOB_LIST_6', { tag: ["@get_job_list"] }, async ({ page }) => {
    ...
    console.log(r.data.response.status == 'UNAUTHORIZED' ? 'STATUS CHECK PASSED ✅' : `STATUS CHECK FAILED ❌ \nEXPECTED STATUS: UNAUTHORIZED \nACTUAL STATUS: ${JSON.stringify(r.data.response.status)}`);
    ...
    expect(r.data.response.status).toEqual('UNAUTHORIZED')
